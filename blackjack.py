...
def load_retroset(card_images):
    suits = ['heart', 'club', 'diamond', 'spade']
    face_cards = ['jack', 'queen', 'king', 'ace']

    for suit in suits:
        for card in range(2, 11):
            name = 'newCards\\{}_{}.{}'.format(str(card), suit, extension)
            image = tkinter.PhotoImage(file=name)
            card_images.append((card, image, suit.upper(), str(card).upper()))

        for card in face_cards:
            name = 'newCards\\{}_{}.{}'.format(str(card), suit, extension)
            image = tkinter.PhotoImage(file=name)
            if str(card) == 'ace':
                card_images.append((1, image, suit.upper(), str(card).upper()))
            else:
                card_images.append((10, image, suit.upper(), str(card).upper()))


def load_casinoset(card_images):
    card_names = {
        1: 'ace',
        2: 'two',
        3: 'three',
    ...
    for suit in suits:
        for card in range(1, 11):
            name = 'casinoSet\\{}_{}.{}'.format(suit, card, extension)
            image = tkinter.PhotoImage(file=name)
            card_images.append((card, image, suit.upper(), card_names[card].upper()))
        for card in face_cards:
            name = 'casinoSet\\{}_{}.{}'.format(suit, card, extension)
            image = tkinter.PhotoImage(file=name)
            card_images.append((10, image, suit.upper(), face_cards[card].upper()))


def load_aqua(card_images):
    ...
    card_back_image = tkinter.PhotoImage(file="aquaset\\back{}.{}".format(random.randint(1, 13), extension))

    for suit in suits:
        for card in range(1, 10):
            name = 'aquaset\\{}0{}.{}'.format(suit, str(card), extension)
            image = tkinter.PhotoImage(file=name)
            card_images.append((card, image, str(suits[suit]).upper(), card_names[card].upper()))

        for card in range(11, 14):
            name = 'aquaset\\{}{}.{}'.format(suit, str(card), extension)
            image = tkinter.PhotoImage(file=name)
...
def clear_table():
    ...
    dealer_card_frame = tkinter.Frame(card_frame, height=139, background=felt_color)
    dealer_card_frame.grid(row=0, column=1, sticky='ew', rowspan=2)
    player_card_frame = tkinter.Frame(card_frame, height=139, background=felt_color)
    ...
    dealer_score_no["font"] = font.Font(size=14)
    player_score_no["font"] = font.Font(size=14)
...
def deal_card(frame, hand):
    ...
    tkinter.Label(frame, image=next_card[1], borderwidth=0, relief='raised',
                  background=felt_color).pack(side='left',
                                              padx=(1, 3), pady=(1, 3))
...
def score_hand(hand):
    ...
    # Only one ace can have a value 11, and this be reduced to 1 if the
    # hand is a bust.
    score = 0
    ace = False
    global ace_hard
...
def deal_dealer():
    ...
    if not player_bj:
        flip_first_card()
    mainWindow.after(gen_delay_time, mainWindow.update())
    dealer_score = score_hand(dealer_hand)
    while 0 < dealer_score <= dealerstand_score:
        if dealer_score == dealerstand_score:
            if dealer_hits_on_soft and 0 < check_for_ace(dealer_hand) <= 2:
                pass
            else:
...
def deal_player():
    ...
    if not player_bj:
        ...
        if player_score > 21:
            ...
            player_lost()
            mainWindow.after(gen_delay_time * 2, mainWindow.update())
            flip_first_card()
            reenable_buttons()
...
def first_play():
    ...
    if not player_bj:
        ...
        result_text.set("Dealer draws his hole card.\t\tYour turn...")
        reenable_buttons()
...
def shuffle():
    ...
    # deck.insert(0, (1, card_back_image, ' sff', 'ace'))
    # deck.insert(0, (5, card_back_image, ' sff', '5'))
    # deck.insert(0, (1, card_back_image, ' sff', '1'))
    # deck.insert(0, (10, card_back_image, ' sff', '5'))
    # deck.insert(0, (3, card_back_image, ' sff', '3'))
    # deck.insert(0, (1, card_back_image, ' sff', '11'))
    first_play()
...
def start_game():
    ...
    PlaySound.PlaySound('sounds\\cardOpenPackage{}.wav'.format(random.randint(1, 2)),
                        PlaySound.SND_NOSTOP + PlaySound.SND_FILENAME)
    ...
    PlaySound.PlaySound('sounds\\cardFan{}.wav'.format(random.randint(1, 2)),
                        PlaySound.SND_NOSTOP + PlaySound.SND_FILENAME + PlaySound.SND_ASYNC)
    shuffle()
    deck_ready = False
    mainWindow.mainloop()
...
# Set up the screen and frames for the dealer and the player  TODO: Make this a separate init_GUI function
mainWindow = tkinter.Tk()
mainWindow.title("Vit's Blackjack Table")
mainWindow.geometry("750x490+265+100")
...
# Drawing the felt
card_frame = tkinter.Frame(mainWindow, relief='ridge', bg=felt_color,
                           borderwidth='3')
...
dealer_score_obj.grid(row=0, column=0, sticky='n', pady=4)
dealer_score_obj.place()
dealer_score_no = tkinter.Label(card_frame,
                                textvariable=dealer_score_label,
                                font=font.Font(size=14),
                                background=felt_color, fg='white')
dealer_score_no.grid(row=1, column=0, sticky='n')
# embedded card_frame to hold the card images
dealer_card_frame = tkinter.Frame(card_frame, height=139, background=felt_color)
dealer_card_frame.grid(row=0, column=1, sticky='ewns', rowspan=2)
# Player's score area
player_score_label = tkinter.IntVar()
player_score_obj = tkinter.Label(card_frame, text='Player Score:',
                                 background=felt_color, fg='white')
player_score_obj.grid(row=2, column=0, sticky='n', pady=4)
player_score_no = tkinter.Label(card_frame,
                                textvariable=player_score_label,
                                font=font.Font(size=14), background=felt_color, fg='white')
player_score_no.grid(row=3, column=0, sticky='n')
# embedded card_frame hold the card images
player_card_frame = tkinter.Frame(card_frame, height=139, background=felt_color)
player_card_frame.grid(row=2, column=1, sticky='ewns', rowspan=2)
mainWindow.update()
# Control panel area
...
mainWindow.update()
mainWindow.minsize(750, button_frame.winfo_height() + result.winfo_height() + card_frame.winfo_height())
mainWindow.maxsize(750, button_frame.winfo_height() + result.winfo_height() + card_frame.winfo_height())
